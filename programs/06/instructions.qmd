...
method. Additionally:
    1. the `number` mutator should only allow integer values between 2 and 10 inclusive i.e. [2,
	10]. If any other value is attempted, the number should be set to 2.


    2. the suit mutator should only allow arguments that match with one of the four possibilities
	in the `POSSIBLESUITS` constant defined at the top of the template file. If any other options
	are attempted, the suit should be set to “clubs”.
...
`deck`, it will:
    1. draw two cards from the `deck` (one for the player and one for the computer),
    2. `shuffle` the deck again, and 
    3. then compare the two cards. 
